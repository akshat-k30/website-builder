...
export default function EditPanel({ activeSection, content, theme, onUpdate }: EditPanelProps) {
  ...
  const commitSkills = (categories: { name: string; items: string[] }[]) =>
    onUpdate("skills", { ...content.skills, categories })

  const setCategoryName = (ci: number, name: string) =>
    commitSkills(skillCategories.map((c, i) => (i === ci ? { ...c, name } : c)))
  const addCategory = () => commitSkills([...skillCategories, { name: "New Category", items: [""] }])
  const removeCategory = (ci: number) => commitSkills(skillCategories.filter((_, i) => i !== ci))
  const setItem = (ci: number, ii: number, value: string) =>
    commitSkills(
      skillCategories.map((c, i) =>
        i === ci ? { ...c, items: c.items.map((it, j) => (j === ii ? value : it)) } : c
      )
    )
  const addItem = (ci: number) =>
    commitSkills(skillCategories.map((c, i) => (i === ci ? { ...c, items: [...c.items, ""] } : c)))
  const removeItem = (ci: number, ii: number) =>
    commitSkills(
      skillCategories.map((c, i) => (i === ci ? { ...c, items: c.items.filter((_, j) => j !== ii) } : c))
    )
        ...
        {activeSection === "skills" && (
          <div className="space-y-5">
            {skillCategories.map((cat, ci) => (
              <div key={ci} className="p-5 bg-card rounded-xl border border-border space-y-3 shadow-sm">
                <div className="flex items-center gap-2">
                  <input
                    ...
                    className="flex-1 p-2.5 text-sm font-bold bg-background border border-border rounded-lg focus:ring-1 focus:ring-primary focus:border-primary text-foreground outline-none transition-colors"
                  />
                  <button
                    onClick={() => removeCategory(ci)}
                    aria-label="Remove category"
                    className="shrink-0 w-9 h-9 flex items-center justify-center rounded-lg text-red-500 hover:bg-red-50 transition-colors"
                  >
                ...
                <div className="space-y-2">
                  {cat.items.map((item, ii) => (
                    <div key={ii} className="flex items-center gap-2">
                      <input
                        ...
                        placeholder="Skill"
                        className="flex-1 p-2 text-sm bg-background border border-border rounded-lg focus:ring-1 focus:ring-primary focus:border-primary text-foreground outline-none transition-colors"
                      />
                      <button
                        onClick={() => removeItem(ci, ii)}
                        aria-label="Remove skill"
                        className="shrink-0 w-8 h-8 flex items-center justify-center rounded-lg text-muted-foreground hover:bg-muted hover:text-red-500 transition-colors"
                      >
                  ...
                  <button
                    onClick={() => addItem(ci)}
                    className="text-xs font-bold text-primary hover:text-primary-hover flex items-center gap-1.5 mt-1 px-2 py-1.5 rounded-lg hover:bg-primary/5 transition-colors"
                  >
                    <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2.5}>
